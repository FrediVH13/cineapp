			...
			<div class="my-3 p-3 bg-white rounded shadow-sm">
				...
				<div class="form-row">
					<div class="col-sm-7 mb-3">
						<div class="media">
							<img src="${urlPublic}/images/movies/${movie.billboard_image}"
								width="155" class="align-self-center mr-3 img-fluid rounded"
								alt="...">
							<div class="media-body">
								<h4 class="text-primary mt-0">${movie.title}</h4>
								<p class="text-muted">${movie.original_title}&nbsp;(${movie.id_country},
									${movie.year})</p>
								<p class="mb-0 font-weight-bold">
									Clasificación: <span class="font-weight-normal">${movie.id_classification}</span>
								</p>
								<p class="mb-0 font-weight-bold">
									Duración: <span class="font-weight-normal">${movie.duration}</span>
								</p>
								<p class="mb-0 font-weight-bold">
									Género: <span class="font-weight-normal">${movie.id_genre}</span>
								</p>
							</div>
						</div>
					</div>
				...
				<c:choose>
					<c:when test="${movie.directors == null && movie.actors == null}"></c:when>
					<c:otherwise>
						<h6 class="text-primary font-weight-bold">
				...
				<c:if test="${movie.actors != null}">
					<div class="row">
						...
						<div class="col-9">
							<p class="text-justify">${movie.actors}</p>
						</div>
					</div>
				</c:if>
				<c:if test="${movie.directors != null}">
					<div class="row">
						<div class="col-3 col-sm-1 font-weight-bold">Directores:</div>
						<div class="col-9">
							<p class="text-justify">${movie.directors}</p>
						</div>
					</div>
				</c:if>
			</div>
